...
for i in range(number_of_airfoils-1):
    ...
    Boundaries.set_boundaries("blade_pressure", profile[i], [6, 7], profile[i + 1], [7, 6])
    Boundaries.set_boundaries("blade_pressure", profile[i], [7, 8], profile[i + 1], [8, 7])
    Boundaries.set_boundaries("blade_pressure", profile[i], [8, 9], profile[i + 1], [9, 8])
    Boundaries.set_boundaries("blade_suction", profile[i], [9, 10], profile[i + 1], [10, 9])
    Boundaries.set_boundaries("blade_suction", profile[i], [10, 11], profile[i + 1], [11, 10])
    Boundaries.set_boundaries("blade_suction", profile[i], [11, 6], profile[i + 1], [6, 11])
    if i < number_of_airfoils-1:
